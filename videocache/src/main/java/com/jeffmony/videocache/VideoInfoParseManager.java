...
public class VideoInfoParseManager {
    ...
    private void parseNetworkM3U8Info(VideoRequest videoRequest, VideoCacheInfo cacheInfo) {
        try {
            release();
            PlayerProgressListenerManager.getInstance().log("parseNetworkM3U8Info");
            ...
            if (m3u8.isIsLive()) {
                ...
                videoRequest.getVideoInfoParsedListener().onM3U8ParsedFailed(new VideoCacheException(m3u8.exception==null?"parseM3U8Info failed, zero ts":m3u8.exception.getMessage()), cacheInfo);
            } else {
...
//                }
    ...
    public void parseProxyM3U8Info(VideoRequest videoRequest, VideoCacheInfo cacheInfo) {
        ...
        if (!proxyM3U8File.exists()) {
            PlayerProgressListenerManager.getInstance().log("m3u8代理文件不存在" );
