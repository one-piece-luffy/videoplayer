...
import static com.baofu.cache.downloader.utils.OkHttpUtil.NO_SPACE;
...
import com.baofu.cache.downloader.utils.MimeType;
...
//                    Log.i(TAG, "文件大小：" + mFileLength);
                ...
                long freeSpace = VideoDownloadUtils.getFreeSpaceBytes(CacheDownloadManager.getInstance().mConfig.privatePath);
...
//            Log.i(TAG, "content-type：" + contentType);
    ...
    private void retryInit(String url, int code, String message) {
        ...
        } else if (mRetryCount < CacheDownloadManager.getInstance().mConfig.retryCount) {
            resetStutus();
            initDownloadInfo(url);
        } else {
...
//        final File cacheFile = new File(mSaveDir, "thread" + threadId + "_" + fileName + ".cache");
        final File cacheFile = new File(VideoStorageUtils.getTempDir(CacheDownloadManager.getInstance().mConfig.context), "thread" + threadId + "_" + fileName + ".cache");
